package h.heiErDing.loader.utils;
...
public class NativeLibraryLoader {
    private static final String TAG = "[heiErDing:NativeLoader]";
    ...
    private String getModuleApkPath(ClassLoader moduleClassLoader) {
            ...
            while (clz != null) {
                for (java.lang.reflect.Field f : KavaReflector.declaredFields(clz)) {
                    ...
                    if (val instanceof String) {
                        String s = (String) val;
                        if (s.endsWith(".apk") && s.contains("h.heiErDing") && new File(s).exists()) {
                            return s;
                        }
                    }
                }
        ...
        try {
            ...
            if (dirs != null) {
                for (File dir : dirs) {
                    if (dir != null && dir.isDirectory() && dir.getName().contains("h.heiErDing")) {
    ...
    private boolean loadLibraryUncached(Context ctx, ClassLoader moduleClassLoader, String soName,
        ...
        try {
            ...
            h.heiErDing.utils.HLog.e(TAG + " " + soName + " 加载失败: " + e.getMessage(), e);
            try {
                System.loadLibrary(libName);
                return true;
            } catch (Throwable e2) {
                h.heiErDing.utils.HLog.e(TAG + " " + libName + " loadLibrary 也失败: " + e2.getMessage(), e2);
